...
import '../../assets/reveal/reset.css';
import '../../assets/reveal/reveal.css';
import '../../assets/reveal/monokai.css';
...
export const RevealSlides = forwardRef<RevealHandle, RevealSlidesProps>(
  (
    {
      theme = 'black',
      presState = { indexh: -1, indexv: -1, indexf: -1, paused: false, overview: false },
      disable = false,
    ...
    useEffect(() => {
      ...
      revealRef.current.initialize().then(() => {
        ...
        if (onStateChange && revealRef.current) {
          const currentRef = revealRef.current;
          // Send slide position indecies back to Streamlit on initialization and on slide change
          onStateChange(currentRef.getState());

          currentRef.on('slidechanged', () => {
            onStateChange(currentRef.getState());
          });
          currentRef.on('fragmentshown', () => {
            // event.fragment = the fragment DOM element
            console.log('fragment shown');
            onStateChange(currentRef.getState());
          });
          currentRef.on('fragmenthidden', () => {
            // event.fragment = the fragment DOM element
            onStateChange(currentRef.getState());
          });
          currentRef.on('overviewshown', () => {
            // event.overview = the overview DOM element
            onStateChange(currentRef.getState());
          });
          currentRef.on('overviewhidden', () => {
            // event.overview = the overview DOM element
            onStateChange(currentRef.getState());
          });
          currentRef.on('paused', () => {
            // event.fragment = the fragment DOM element
            onStateChange(currentRef.getState());
          });
          currentRef.on('resumed', () => {
            // event.fragment = the fragment DOM element
            onStateChange(currentRef.getState());
          });
        }
        ...
        if (Object.keys(presState).length !== 0 && revealRef.current) {
    ...
    useLayoutEffect(() => {
      ...
      const themeImport = async () => {
        try {
          await import(`../../assets/reveal/theme/${theme}.css`);
        } catch (e) {
        ...
        if (revealRef.current) {
          try {
            ...
            console.warn('Reveal.layout() call failed.', e);
            setVisible(true);
          }
        }
      };
    ...
    return (
      <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
        <div
          className="reveal"
          ref={revealDivRef}
          style={{ opacity: visible ? 1 : 0, transition: transitionOnThemeLoaded }}
        >
